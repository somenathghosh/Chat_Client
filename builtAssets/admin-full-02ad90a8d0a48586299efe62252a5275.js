...
socket.on('New Client', function(data) {
	console.log('400');
	$('.chat-area').append(newChatContainer(data.roomID, data.person, "Company"));
	$('.sidebar').append(newSidebarChat(data.roomID, data.person, "Company"));
	$inputMessage = $('#' + data.roomID);
	$chatMessages = $('#chat' + data.roomID).append(newTimestamp('Chat Start'))
